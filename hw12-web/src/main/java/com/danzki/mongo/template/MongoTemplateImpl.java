...
public class MongoTemplateImpl implements MongoTemplate {
  private static Logger logger = LoggerFactory.getLogger(MongoTemplateImpl.class);
  ...
  public <T> Optional<T> findOne(Bson query, Class<T> tClass) {
    ...
    return Optional.ofNullable(document).map(d -> {
      try {
        return document2Object(d, tClass);
      } catch (IOException e) {
        e.printStackTrace();
      }
      return null;
  ...
  @Override
  public <T> List<T> find(Bson query, Class<T> tClass) throws IOException {
  ...
  private <T> T document2Object(Document document, Class<T> tClass) throws IOException {
